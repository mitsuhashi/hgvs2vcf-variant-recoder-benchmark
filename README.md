...
- REF配列の重なる範囲から同一alleleと証明できるleft-align・最小表現の差

完全一致に加えて、両VCFを共通の参照配列断片へ適用した結果が一致する場合も合格と
します。参照配列断片をVCFだけから再構成できない場合は、同じ染色体・同じ長さの変化
という理由だけでは合格にしません。transcript、gene、`ambiguous` の表記差は合否に
影響しません。
